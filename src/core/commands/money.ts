...
const money: CustomCommand = {
  ...
  aliases: ['m'],
  usage: [
    { description: '(별칭: ls) 최근 n개의 트랜잭션을 보여줍니다. 이름(들)이 주어질 경우 해당 인물(들)과 관계된 트랜잭션만 보여집니다.', args: 'list <n> [이름] [이름]' },
    { description: '(별칭: st) 현재 채무 상태를 보여줍니다. 이름이 주어질 경우 해당 인물의 채무 상태를 보여줍니다.', args: 'status [이름]' },
    { description: '(별칭: t) 트랜잭션을 추가합니다', args: 'transaction <금액(원)> <준 사람> <받은 사람> [사유]' },
    { description: '(별칭: d) 더치페이 정보를 추가합니다', args: 'dutch <총 금액(원)> <돈 낸 사람> <돈 낸 사람 제외 더치페이 참여자 목록(쉼표 구분)> [사유]' },
  ...
  execute(message, argv) {
    ...
    } else if (argv[1] === 'status' || argv[1] === 'st') {
